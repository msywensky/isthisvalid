...
/**
 * Normalised result shape any SMTP-verification provider (ZeroBounce,
 * Emailable, ...) reduces to. Defined here — rather than in the app's
 * server-only smtp-provider.ts — because this package has no dependency on
 * that server-only code; smtp-provider.ts imports it from here instead.
 */
